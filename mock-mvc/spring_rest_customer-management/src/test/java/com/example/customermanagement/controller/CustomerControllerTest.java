...
public class CustomerControllerTest {
	...
	public void testUpdateCustomer() throws Exception {
		...
		when(customerService.getCustomerById(1)).thenReturn(Optional.of(customer));
		when(customerService.updateCustomer(1,any(Customer.class))).thenReturn(ResponseEntity.ok(customer));
		...
		mockMvc.perform(put("/api/customers/1").contentType(MediaType.APPLICATION_JSON).content(
				...
				.andExpect(status().isOk()).andExpect(jsonPath("$.firstName").value("Bob"))
